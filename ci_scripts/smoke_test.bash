...
IFS="," SMOKE_TEST_SERVERS_ARRAY=(${SMOKE_TEST_SERVERS}); for i in "${SMOKE_TEST_SERVERS_ARRAY[@]}"; do
  ...
  if grep -q AUTHENTICATION_SUCCESS_EVENT "$REPORT_FNAME".$i.smoke.log; then echo success; else
    ((FAILED++))
    echo failed
  fi
  if [ $(cat "$REPORT_FNAME".$i.smoke.log | grep -c ERROR ) = "0" ]; then echo success; else
      ((FAILED++))
      echo failed
  fi
done
