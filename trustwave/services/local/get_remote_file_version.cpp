/*
 * get_remote_file_size.cpp
 *
 *  Created on: May 23, 2019
 *      Author: root
 */

#include "get_remote_file_version.hpp"

#include "../../authenticated_scan_server.hpp"
#include "protocol/msg_types.hpp"
using namespace trustwave;
int Get_Remote_File_Version::act(const header& header,std::shared_ptr<action_msg> action, std::shared_ptr<result_msg> res)
{
    res->id(action->id());
...
Dispatcher<Action_Base>::Registrator Get_Remote_File_Version::m_registrator(new Get_Remote_File_Version,authenticated_scan_server::instance().public_dispatcher);
