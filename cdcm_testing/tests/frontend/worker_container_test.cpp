...
#include "../../src/frontend/worker_container.hpp"
...
struct OneInContainer: public EmptyContainer {
    OneInContainer() { workers.insert(std::make_shared<worker>("worker1")); }
};
...
struct TwoInContainer: public OneInContainer {
    TwoInContainer() { workers.insert(std::make_shared<worker>("worker2")); }
};
...
BOOST_FIXTURE_TEST_CASE(exists, OneInContainer) { BOOST_TEST(workers.exists(std::string("worker1"))); }
...
BOOST_FIXTURE_TEST_CASE(erase_not_exists, EmptyContainer) { BOOST_TEST(!workers.erase("worker")); }
