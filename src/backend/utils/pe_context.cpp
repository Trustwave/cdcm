...
pe_context::pe_context(file_reader_interface& fr) : fm_(fr) {}
...
int pe_context::parse() {
    ...
    const uint32_t *signature_ptr = ptr_add<uint32_t>(pe_.dos_hdr,
                                                      pe_.dos_hdr->e_lfanew);

    if (!fm_.map_chunk_by_pointer(signature_ptr, LIBPE_SIZEOF_MEMBER(pe_file_t, signature))) {
        return LIBPE_E_INVALID_LFANEW;//todo assaf 4
    }
    ...
    if (!fm_.map_chunk_by_pointer(opt_type_ptr, LIBPE_SIZEOF_MEMBER(IMAGE_OPTIONAL_HEADER, type)))
        return LIBPE_E_MISSING_OPTIONAL_HEADER;//todo assaf 2
    ...
    switch (pe_.optional_hdr.type) {
        ...
        case MAGIC_PE32:
            if (!fm_.map_chunk_by_pointer(pe_.optional_hdr_ptr,
                ...
                return LIBPE_E_MISSING_OPTIONAL_HEADER;//todo assaf 96
            pe_.optional_hdr._32 = reinterpret_cast<IMAGE_OPTIONAL_HEADER_32 *>( pe_.optional_hdr_ptr);
            pe_.optional_hdr.length = sizeof(IMAGE_OPTIONAL_HEADER_32);
            pe_.num_directories =
                    pe_.optional_hdr._32->NumberOfRvaAndSizes;
            pe_.entrypoint = pe_.optional_hdr._32->AddressOfEntryPoint;
            pe_.imagebase = pe_.optional_hdr._32->ImageBase;
            break;
        case MAGIC_PE64:
            if (!fm_.map_chunk_by_pointer(pe_.optional_hdr_ptr,
                                          sizeof(IMAGE_OPTIONAL_HEADER_64)))
                return LIBPE_E_MISSING_OPTIONAL_HEADER;//todo assaf 112
            pe_.optional_hdr._64 = reinterpret_cast<IMAGE_OPTIONAL_HEADER_64 *>( pe_.optional_hdr_ptr);
            pe_.optional_hdr.length = sizeof(IMAGE_OPTIONAL_HEADER_64);
            pe_.num_directories =
                    pe_.optional_hdr._64->NumberOfRvaAndSizes;
            pe_.entrypoint = pe_.optional_hdr._64->AddressOfEntryPoint;
            pe_.imagebase = pe_.optional_hdr._64->ImageBase;
            break;
    }

    if (pe_.num_directories > MAX_DIRECTORIES)/*16*/ {
        //fprintf(stderr, "Too many directories (%u)\n", pe_.num_directories);
        return LIBPE_E_TOO_MANY_DIRECTORIES;
    }

    if (pe_.num_sections > MAX_SECTIONS)/*96*/ {
    ...
    if (!fm_.map_chunk_by_pointer(pe_.directories_ptr,
        ...
        return LIBPE_E_ALLOCATION_FAILURE;//todo assaf 2 fixe ret
    ...
    if (!fm_.map_chunk_by_pointer(pe_.sections_ptr, sizeof(IMAGE_SECTION_HEADER) * pe_.num_sections))
        return LIBPE_E_ALLOCATION_FAILURE;//todo assaf 2 fixe ret
    if (pe_.num_directories > 0) {
        pe_.directories = malloc_helper<IMAGE_DATA_DIRECTORY *>(pe_.num_directories);
        if (pe_.directories == nullptr)
            return LIBPE_E_ALLOCATION_FAILURE;
        for (uint32_t i = 0; i < pe_.num_directories; i++) {
            pe_.directories[i] = ptr_add<IMAGE_DATA_DIRECTORY>(pe_.directories_ptr,
                                                               i * sizeof(IMAGE_DATA_DIRECTORY));
        }
    } else {
        pe_.directories_ptr = nullptr;
    }

    if (pe_.num_sections > 0) {
        pe_.sections = malloc_helper<IMAGE_SECTION_HEADER *>(pe_.num_sections);
        if (pe_.sections == nullptr)
            return LIBPE_E_ALLOCATION_FAILURE;
        for (uint32_t i = 0; i < pe_.num_sections; i++) {
            pe_.sections[i] = ptr_add<IMAGE_SECTION_HEADER>(pe_.sections_ptr,
                                                            i * sizeof(IMAGE_SECTION_HEADER));
        }
    } else {
        pe_.sections_ptr = nullptr;
    }
    return 0;
}
...
void pe_context::showVersion() {
   ...
   // VS_FIXEDFILEINFO *info = reinterpret_cast<VS_FIXEDFILEINFO *>(const_cast<char *> (buffer));
    ...
    WORD *padding = ptr_add<WORD>(vih, sizeof(version_info_header)+string_file_version_len);
    ...
    WORD *st_padding = ptr_add<WORD>(st_vih, sizeof(version_info_header)+string_table_key_len);
    auto *str_vih = ptr_add<version_info_header>(st_padding,
                                                                calculate_padding(st_padding, fm_.data()));//

   // char16_t * unicode1 = ptr_add<char16_t>(st_vih, sizeof(version_info_header));
    const char *buffer_end = ptr_add<char>(buffer,dataEntrySize );
