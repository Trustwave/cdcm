...
namespace trustwave {
    ...
    struct local_start_session_msg: public action_msg {
        ...
        std::string remote;
        std::optional<std::vector<protocol_creds>> protocol_credentials;
    };
} // namespace trustwave
namespace tao::json {
    template<>
    struct traits<trustwave::protocol_creds>:
        ...
        > {
        TAO_JSON_DEFAULT_KEY("credentials");
        ...
        static trustwave::protocol_creds as(const tao::json::basic_value<Traits>& v)
        {
            ...
            result.authenticate = object.at("authenticate").template as<std::string>()=="true"?true:false;
    ...
    static trustwave::local_start_session_msg as(const tao::json::basic_value<Traits>& v)
    {
        ...
        if(object.find("credentials")) {
            //            auto credentials_array = msg_object.at("credentials").get_array();
            result.protocol_credentials = object.at("credentials").template as<std::vector<trustwave::protocol_creds>>();
            //            for(auto creds: credentials_array) {
            //                auto creds_obj = creds.get_object();
            //            }
        }
        return result;
    }
