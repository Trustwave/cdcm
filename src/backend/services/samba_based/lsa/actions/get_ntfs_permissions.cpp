...
#include "get_ntfs_permissions.hpp"
...
//                          						namespaces
//=====================================================================================================================
using trustwave::Get_NTFS_Permissions_Action;
...
void split_share_path(const std::string& full, std::string& share, std::string& path)
{
    ...
    if(std::string::npos != pos) {
        path.assign(path.substr(pos + 1));
        share.assign(path.substr(0, pos));
    }
    else {
        share.assign(path);
    }
}
action_status Get_NTFS_Permissions_Action::act(boost::shared_ptr<session> sess, std::shared_ptr<action_msg> action,
                                                   std::shared_ptr<result_msg> res)
{
    if(!sess || (sess && sess->id().is_nil())) {
        ...
        auto gnpact = std::dynamic_pointer_cast<lsa_action_get_ntfs_permissions_msg>(action);
        if(!gnpact) {
    ...
    if(share.empty()||path.empty()) {
        res->res("Error: share and path needed");
        return action_status::FAILED;
    }
    struct my_frame
    {
    ...
    }ff;
    {
        ...
        auto ret = c.get_sd("",trustwave::sd_utils::entity_type::GENERIC,sd);
...
static std::shared_ptr<Get_NTFS_Permissions_Action> instance = nullptr;

// extern function, that declared in "action.hpp", for export the plugin from dll
std::shared_ptr<trustwave::Action_Base> import_action()
{
    return instance ? instance : (instance = std::make_shared<Get_NTFS_Permissions_Action>());
}