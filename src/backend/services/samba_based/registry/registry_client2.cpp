...
NTSTATUS registry_openkey(TALLOC_CTX* mem_ctx, struct rpc_pipe_client* pipe_hnd, const std::string& name, uint32_t access_mask,
                          struct policy_handle* hive_hnd, struct policy_handle* key_hnd)
{
    uint32_t hive;
    std::string keyname;
...
NTSTATUS trustwave::registry_client2::open_key(const std::string& k)
{
    ...
    NTSTATUS status
        = registry_openkey(mem_ctx_, client_->pipe_handle(), k, access_mask, &ctx_.pol_hive, &ctx_.pol_key);
    if(!NT_STATUS_IS_OK(status)) {
...
NTSTATUS trustwave::registry_client2::get_sd(trustwave::sd_utils::Security_Descriptor_str &outsd)
{
    NTSTATUS status;
    ...
    ndr_err_code ndr_err
        = ndr_pull_struct_blob(&blob, mem_ctx_, &sec_desc, (ndr_pull_flags_fn_t)ndr_pull_security_descriptor);
...
trustwave::registry_client2::~registry_client2() { talloc_free(mem_ctx_); }
