...
namespace trustwave {
    ...
    struct worker {
        ...
        void dump() const
        {
            ...
            std::cerr
                << "Expiry in : "
                << std::chrono::duration_cast<std::chrono::milliseconds>(expiry_ - chr::system_clock::now()).count()
    ...
    public:
        ...
        explicit worker_container(std::chrono::seconds heartbeat_expiry): heartbeat_expiry_(heartbeat_expiry) {}
        ...
        [[nodiscard]] bool is_busy(const std::string& identity) const { return !is_idle(identity); }

        bool set_idle(const std::string& identity) { return modify_idle(identity, true); }
        bool set_busy(const std::string& identity) { return modify_idle(identity, false); }
        ...
        bool update_expiration(const std::string& identity)
        {
            ...
            return cont_.get<expiration>().modify(
                it2, [this](sp_worker_t x) { x->expiry_ = chr::system_clock::now() + heartbeat_expiry_; });
        }
        void dump()
        {
            std::cerr << "================================================DUMP========================================"
    ...
    private:
        ...
        struct idle_ids_compare {
            ...
            bool operator()(const std::string& x, const worker& y) const { return compare(x, y.identity_); }
            bool operator()(const worker& x, const std::string& y) const { return compare(x.identity_, y); }

            template<typename T0> bool operator()(const std::tuple<T0>& x, const worker& y) const
            {
                return compare(std::get<0>(x), y.identity_);
            }
            template<typename T0> bool operator()(const worker& x, const std::tuple<T0>& y) const
            {
                return compare(x.identity_, std::get<0>(y));
            }
            template<typename T0, typename T1> bool operator()(const std::tuple<T0, T1>& x, const worker& y) const
            {
                return compare(std::get<0>(x), std::get<1>(x), y.identity_, y.idle_);
            }
            template<typename T0, typename T1> bool operator()(const worker& x, const std::tuple<T0, T1>& y) const
            {
        ...
        using workers_t = bmi::multi_index_container<
            ...
            bmi::indexed_by<
                ...
                bmi::ordered_non_unique<
                    bmi::tag<expiration>,
                    bmi::member<worker, std::chrono::time_point<chr::system_clock>, &worker::expiry_>, std::greater<>>,
