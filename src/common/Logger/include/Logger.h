...
#define severity_levels_LIST(m) \
    m(severity_levels, debug1, 1 << 0) m(severity_levels, debug2, 1 << 1) m(severity_levels, info, 1 << 2) \
        m(severity_levels, warning, 1 << 3) m(severity_levels, error, 1 << 4)
...
#define sinks_LIST(m) m(sinks, file, 1 << 0) m(sinks, event_log, 1 << 1) m(sinks, output_debug_string, 1 << 2)
    ...
    public:
        ...
        trustwave::logger::sources get_source() { return source_; }
