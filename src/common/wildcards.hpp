...
//                          						Include files
//=====================================================================================================================
#ifndef SRC_COMMON_WILDCARDS_HPP
#define SRC_COMMON_WILDCARDS_HPP
...
#include <utility>
namespace wildcards {
    template<typename T> struct cards {
        ...
        constexpr cards(T a, T s, T e, T so, T sc, T sn, T ao, T ac, T ar):
            ...
            set_enabled{true}, set_open{std::move(so)}, set_close{std::move(sc)}, set_not{std::move(sn)},
            alt_enabled{true}, alt_open{std::move(ao)}, alt_close{std::move(ac)}, alt_or{std::move(ar)}
        {
        }
        T anything;
        T single;
    ...
    template<> struct cards<char16_t> {
        constexpr cards(cards_type type = cards_type::extended):
            set_enabled{type == cards_type::extended}, alt_enabled{type == cards_type::extended}
        {
        }
        ...
        constexpr cards(char16_t a, char16_t s, char16_t e, char16_t so, char16_t sc, char16_t sn, char16_t ao,
                        char16_t ac, char16_t ar):
            anything{std::move(a)},
            single{std::move(s)}, escape{std::move(e)},
            set_enabled{true}, set_open{std::move(so)}, set_close{std::move(sc)}, set_not{std::move(sn)},
            alt_enabled{true}, alt_open{std::move(ao)}, alt_close{std::move(ac)}, alt_or{std::move(ar)}
        {
        }
    ...
    template<> struct cards<char32_t> {
        constexpr cards(cards_type type = cards_type::extended):
            set_enabled{type == cards_type::extended}, alt_enabled{type == cards_type::extended}
        {
        }
        ...
        constexpr cards(char32_t a, char32_t s, char32_t e, char32_t so, char32_t sc, char32_t sn, char32_t ao,
                        char32_t ac, char32_t ar):
            anything{std::move(a)},
            single{std::move(s)}, escape{std::move(e)},
            set_enabled{true}, set_open{std::move(so)}, set_close{std::move(sc)}, set_not{std::move(sn)},
            alt_enabled{true}, alt_open{std::move(ao)}, alt_close{std::move(ac)}, alt_or{std::move(ar)}
    ...
    template<typename T> constexpr cards<T> make_cards(T&& a, T&& s, T&& e)
    {
...
namespace cx {
    template<typename It> constexpr It next(It it) { return it + 1; }
    template<typename It> constexpr It prev(It it) { return it - 1; }
    ...
    template<typename T, std::size_t N> constexpr bool empty(const T (&)[N]) { return false; }
    template<typename E> constexpr bool empty(std::initializer_list<E> il) { return il.size() == 0; }
    ...
    template<typename T, std::size_t N> constexpr T* begin(T (&array)[N]) { return &array[0]; }
    template<typename E> constexpr const E* begin(std::initializer_list<E> il) { return il.begin(); }
    template<typename C> constexpr auto cbegin(const C& c) -> decltype(cx::begin(c)) { return cx::begin(c); }
    ...
    template<typename T, std::size_t N> constexpr T* end(T (&array)[N]) { return &array[N]; }
    template<typename E> constexpr const E* end(std::initializer_list<E> il) { return il.end(); }
    template<typename C> constexpr auto cend(const C& c) -> decltype(cx::end(c)) { return cx::end(c); }
} // namespace cx
#endif
...
#include <utility>
namespace wildcards {
    template<typename C> struct const_iterator {
        using type = typename std::remove_cv<
            typename std::remove_reference<decltype(cx::cbegin(std::declval<C>()))>::type>::type;
    };
    template<typename C> using const_iterator_t = typename const_iterator<C>::type;
    ...
    template<typename C> using iterator_t = typename iterator<C>::type;
    template<typename It> struct iterated_item {
        using type = typename std::remove_cv<typename std::remove_reference<decltype(*std::declval<It>())>::type>::type;
    };
    template<typename It> using iterated_item_t = typename iterated_item<It>::type;
    template<typename C> struct container_item {
        using type = typename std::remove_cv<
            typename std::remove_reference<decltype(*cx::begin(std::declval<C>()))>::type>::type;
    };
    template<typename C> using container_item_t = typename container_item<C>::type;
} // namespace wildcards
#endif
namespace wildcards {
    template<typename SequenceIterator, typename PatternIterator> struct full_match_result {
        bool res;
    ...
    namespace detail {
        template<typename SequenceIterator, typename PatternIterator> struct match_result {
            bool res;
            SequenceIterator s;
            PatternIterator p;
            constexpr operator bool() const { return res; }
        ...
        template<typename T> constexpr T throw_invalid_argument(T t, const char* what_arg)
        {
        ...
        template<typename T> constexpr T throw_logic_error(T t, const char* what_arg)
        {
        ...
        constexpr bool
        is_set(PatternIterator p, PatternIterator pend,
                   ...
                   && (state == is_set_state::open
                           ...
                           : state == is_set_state::not_or_first
                                 ...
                                 : state == is_set_state::first
                                       ? is_set(cx::next(p), pend, c, is_set_state::next)
                                       : state == is_set_state::next
                                             ? *p == c.set_close || is_set(cx::next(p), pend, c, is_set_state::next)
        ...
        enum class match_set_state { open, not_or_first_in, first_out, next_in, next_out };
        ...
        template<typename PatternIterator>
        constexpr bool
        is_alt(PatternIterator p, PatternIterator pend,
               const cards<iterated_item_t<PatternIterator>>& c = cards<iterated_item_t<PatternIterator>>(),
                           ...
                           : state == is_alt_state::next
                                 ? *p == c.escape
                                       ...
                                       : c.set_enabled && *p == c.set_open
                                                 && is_set(cx::next(p), pend, c, is_set_state::not_or_first)
                                             ? is_alt(set_end(cx::next(p), pend, c, set_end_state::not_or_first), pend,
                                                      c, state, depth)
                                             : *p == c.alt_open
                                                   ? is_alt(cx::next(p), pend, c, state, depth + 1)
                                                   : *p == c.alt_close
                                                         ? depth == 1 || is_alt(cx::next(p), pend, c, state, depth - 1)
                       ...
                       : p == pend
                             ...
                             : state == alt_sub_end_state::next
                                   ? *p == c.escape
                                         ...
                                         : c.set_enabled && *p == c.set_open
                                                   && is_set(cx::next(p), pend, c, is_set_state::not_or_first)
                                               ? alt_sub_end(set_end(cx::next(p), pend, c, set_end_state::not_or_first),
                                                             pend, c, state, depth)
                                               : *p == c.alt_open
        ...
        template<typename SequenceIterator, typename PatternIterator, typename EqualTo = cx::equal_to<void>>
        constexpr match_result<SequenceIterator, PatternIterator>
        match_alt(SequenceIterator s, SequenceIterator send, PatternIterator p1, PatternIterator p1end,
                  ...
                  const EqualTo& equal_to = EqualTo(), bool partial = false)
        {
            ...
            return match(s, send, p1, p1end, c, equal_to, true)
                       ...
                       : cx::next(p1end) == p2
                             ...
                             : match_alt(s, send, cx::next(p1end), alt_sub_end(cx::next(p1end), p2, c), p2, p2end, c,
                                         equal_to, partial);
#endif
        }
              ...
              const cards<iterated_item_t<PatternIterator>>& c, const EqualTo& equal_to, bool partial, bool escape)
        {
            ...
            if(c.set_enabled && *p == c.set_open && is_set(cx::next(p), pend, c, is_set_state::not_or_first)) {
                ...
                return match(cx::next(s), send, set_end(cx::next(p), pend, c, set_end_state::not_or_first), pend, c,
                             equal_to, partial);
            }
            if(c.alt_enabled && *p == c.alt_open && is_alt(cx::next(p), pend, c, is_alt_state::next, 1)) {
                ...
                return match_alt(s, send, cx::next(p), alt_sub_end(cx::next(p), p_alt_end, c), p_alt_end, pend, c,
                                 equal_to, partial);
            }
            if(s == send || !equal_to(*s, *p)) {
                return make_match_result(false, s, p);
                                                     ...
                                                     : c.alt_enabled && *p == c.alt_open
                                                           ...
                                                           ? match_alt(
                                                               ...
                                                               alt_sub_end(cx::next(p),
                                                                           ...
                                                                           c),
                                                               alt_end(cx::next(p), pend, c, alt_end_state::next, 1),
                                                               pend, c, equal_to, partial)
                                                           : s == send || !equal_to(*s, *p)
                                                                 ? make_match_result(false, s, p)
...
namespace cx {
    ...
    public:
        ...
        template<std::size_t N> constexpr basic_string_view(const T (&str)[N]): data_{&str[0]}, size_{N - 1} {}
    ...
    template<typename T> constexpr bool operator==(const basic_string_view<T>& lhs, const basic_string_view<T>& rhs)
    {
    ...
    template<typename T> std::basic_ostream<T>& operator<<(std::basic_ostream<T>& o, const basic_string_view<T>& s)
    {
        o << s.data();
        return o;
    }
    template<typename T, std::size_t N> constexpr basic_string_view<T> make_string_view(const T (&str)[N])
    {
        return {str, N - 1};
    }
    template<typename T> constexpr basic_string_view<T> make_string_view(const T* str, std::size_t s)
    {
        return {str, s};
    }
    ...
    namespace literals {
        constexpr string_view operator"" _sv(const char* str, std::size_t s) { return {str, s}; }
        constexpr u16string_view operator"" _sv(const char16_t* str, std::size_t s) { return {str, s}; }
        constexpr u32string_view operator"" _sv(const char32_t* str, std::size_t s) { return {str, s}; }
        constexpr wstring_view operator"" _sv(const wchar_t* str, std::size_t s) { return {str, s}; }
    } // namespace literals
} // namespace cx
#endif
namespace wildcards {
    template<typename Pattern, typename EqualTo = cx::equal_to<void>> class matcher {
    public:
        constexpr explicit matcher(Pattern&& pattern,
                                   const cards<container_item_t<Pattern>>& c = cards<container_item_t<Pattern>>(),
                                   const EqualTo& equal_to = EqualTo()):
            p_{cx::cbegin(pattern)},
            pend_{cx::cend(std::forward<Pattern>(pattern))}, c_{c}, equal_to_{equal_to}
        {
        }
        constexpr matcher(Pattern&& pattern, const EqualTo& equal_to):
            p_{cx::cbegin(pattern)}, pend_{cx::cend(std::forward<Pattern>(pattern))},
            c_{cards<container_item_t<Pattern>>()}, equal_to_{equal_to}
        {
        }
        ...
        {
            return detail::make_full_match_result(cx::cbegin(sequence), cx::cend(sequence), p_, pend_,
                                                  detail::match(cx::cbegin(sequence),
    ...
    namespace literals {
        constexpr auto
        operator"" _wc(const char* str, std::size_t s) -> decltype(make_matcher(cx::make_string_view(str, s + 1)))
        {
            return make_matcher(cx::make_string_view(str, s + 1));
        }
        constexpr auto
        operator"" _wc(const char16_t* str, std::size_t s) -> decltype(make_matcher(cx::make_string_view(str, s + 1)))
        {
            return make_matcher(cx::make_string_view(str, s + 1));
        }
        constexpr auto
        operator"" _wc(const char32_t* str, std::size_t s) -> decltype(make_matcher(cx::make_string_view(str, s + 1)))
        {
            return make_matcher(cx::make_string_view(str, s + 1));
        }
        constexpr auto
        operator"" _wc(const wchar_t* str, std::size_t s) -> decltype(make_matcher(cx::make_string_view(str, s + 1)))
        {
            return make_matcher(cx::make_string_view(str, s + 1));
        }
    } // namespace literals
...
#endif // SRC_COMMON_WILDCARDS_HPP