...
namespace trustwave {
    ...
    public:
        String s_id() const { return session_.uuid_; }
        String s_remote() const { return session_.remote_; }
        ...
        const shared_mem_session& session() const { return session_; }
    ...
    private:
        struct expiration;
        friend std::ostream& ::trustwave::operator<<(std::ostream& os,
                                                     const trustwave::shared_mem_sessions_cache& cache);
        ...
        typedef bmi::multi_index_container<
            ...
            bmi::indexed_by<
                bmi::ordered_unique<boost::multi_index::tag<id>, bmi::const_mem_fun<shared_mem_session_element, String,
                ...
                bmi::ordered_unique<
                    ...
                    bmi::const_mem_fun<shared_mem_session_element, String, &shared_mem_session_element::s_remote>>>,
            typename bip::managed_shared_memory::allocator<shared_mem_session_element>::type>
            sessions;
    ...
    private:
        ...
        sp_session_t update(decltype(::std::declval<typename sessions::index<Tag>::type>().find(String{})) s)
        {
            ...
            map_->get<expiration>().modify(it2, [this](shared_mem_session_element& x) {
                x.expiration_time_ = chr::system_clock::to_time_t(chr::system_clock::now()) + session_idle_timeout_;
            });
    ...
    public:
        ...
        template<typename Tag> sp_session_t get_session_by(const std::string& kv)
        {
        ...
        template<typename Tag> bool touch_by(const std::string& kv)
        {
            auto& idx = map_->get<Tag>();
            decltype(idx.find(String{})) s;
            {
